...
public class SearchHistory {
    ...
    public void updateHistory(
            Board board, Move bestMove, SearchedMove[] quiets, SearchedMove[] captures, boolean white, int depthBonus, int ply, SearchStack ss) {
        ...
        if (!bestMoveCapture) {
            ...
            for (SearchedMove quiet : quiets) {
                // If the best move was quiet, give it a boost in the quiet history table, and penalise all other quiets.
                updateQuietHistory(board, quiet, bestMove, ss, white, depthBonus, ply);
            }
        }

        for (SearchedMove capture : captures) {
            ...
            updateCaptureHistory(board, capture, bestMove, white, depthBonus);
        }

    }

    private void updateQuietHistory(Board board, SearchedMove quietMove, Move bestMove, SearchStack ss, boolean white, int depthBonus, int ply) {
        // For quiet moves we update both the standard quiet and continuation history tables
        if (quietMove == null)
            return;
        boolean good = quietMove.move.equals(bestMove);
        Piece piece = board.pieceAt(quietMove.move.from());
        int depth = Math.max(1, quietMove.depth + depthBonus);
        quietHistoryTable.update(quietMove.move, piece, depth, white, good);
        for (int prevPly : config.contHistPlies()) {
            ...
            if (prevEntry != null && prevEntry.currentMove != null) {
                ...
                contHistTable.update(prevMove, prevPiece, quietMove.move, piece, depth, white, good);
            }
        }
    }

    private void updateCaptureHistory(Board board, SearchedMove captureMove, Move bestMove, boolean white, int depthBonus) {
        if (captureMove == null)
            return;
        boolean good = captureMove.move.equals(bestMove);
        Piece piece = board.pieceAt(captureMove.move.from());
        Piece captured = captureMove.move.isEnPassant() ? Piece.PAWN : board.pieceAt(captureMove.move.to());
        int depth = Math.max(1, captureMove.depth + depthBonus);
        captureHistoryTable.update(piece, captureMove.move.to(), captured, depth, white, good);
    }
