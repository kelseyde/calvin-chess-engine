...
public class Searcher implements Search {
    ...
    public int search(int depth, int ply, int alpha, int beta, boolean cutNode) {
        ...
        while (true) {
            ...
            if (!pvNode && !rootNode) {
                ...
                final int seeThreshold = seeThreshold(depth, historyScore, isQuiet, improving);
    ...
    private int seeThreshold(int depth, int historyScore, boolean quiet, boolean improving) {
        if (quiet && improving) {
            depth++;
        }
        int threshold = quiet ?
