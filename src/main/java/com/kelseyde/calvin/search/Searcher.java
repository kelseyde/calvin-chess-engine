...
//        System.out.printf("max depth: %s, eval: %s, move: %s%n", currentDepth, result.eval(), NotationUtils.toNotation(result.move()));
    ...
    int quiescenceSearch(int alpha, int beta, int depth, int plyFromRoot) {
        ...
        for (Move move : orderedMoves) {
            // Static exchange evaluation: skip captures that are likely to lose material
            int seeEval = see.evaluate(board, move);
            if ((depth <= 3 && seeEval < 0) || (depth > 3 && seeEval <= 0)) {
                continue;
            }

            // Futility pruning: skip captures that likely have no chance of raising alpha
            Piece capturedPiece = move.isEnPassant() ? Piece.PAWN : board.pieceAt(move.getEndSquare());
            if (capturedPiece != null) {
                int delta = standPat + PieceValues.valueOf(capturedPiece) + DELTA_PRUNING_MARGIN;
