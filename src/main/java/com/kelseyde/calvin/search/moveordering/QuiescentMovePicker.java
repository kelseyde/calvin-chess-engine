...
public class QuiescentMovePicker implements MovePicker {
     ...
     */
    public QuiescentMovePicker(MoveGeneration moveGenerator, Board board) {
    ...
    public void scoreMoves(List<Move> stagedMoves) {
        ...
        for (int i = 0; i < stagedMoves.size(); i++) {
            moves[i] = new ScoredMove(stagedMoves.get(i), MvvLva.score(board, stagedMoves.get(i), ttMove));
        }
    }
