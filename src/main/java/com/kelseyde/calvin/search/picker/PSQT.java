...
public class PSQT {
    ...
    private static final int[][] MG_TABLES = new int[][] {
            MG_PAWN_TABLE, MG_KNIGHT_TABLE, MG_BISHOP_TABLE, MG_ROOK_TABLE, MG_QUEEN_TABLE, MG_KING_TABLE
    };

    private static final int[][] EG_TABLES = new int[][] {
            EG_PAWN_TABLE, EG_KNIGHT_TABLE, EG_BISHOP_TABLE, EG_ROOK_TABLE, EG_QUEEN_TABLE, EG_KING_TABLE
    };

    public static int score(Piece piece, int square, boolean white, float phase) {
        int pieceIndex = piece.ordinal();
        square = white ? square : square ^ 56;
        int mgScore = MG_TABLES[pieceIndex][square];
        int egScore = EG_TABLES[pieceIndex][square];
        return (int) (phase * mgScore) + (int) ((1 - phase) * egScore);
    }
