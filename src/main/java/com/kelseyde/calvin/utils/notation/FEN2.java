...
public record FEN2(String value, long[] bitboards, int enPassantFile, boolean whiteToMove, int castleRights, int halfMoveClock, int fullMoveNumber) {
    ...
    public static FEN2 startpos() {
        return FEN2.parse(FEN.STARTPOS);
    }

    public static FEN2 parse(String fen) throws InvalidFenException {
    ...
    public static FEN2 fromBoard(Board board) {
        try {
    ...
    private static int findRook(long rooks, boolean white, boolean kingside) {
        ...
        if (Bits.count(firstRankRooks) == 0)
            throw new FEN.InvalidFenException("Castling rights with no rooks on the first rank!");
    ...
    private static class FenPatterns {
        ...
        public static final Pattern CASTLE      = Pattern.compile("^(-|K?Q?k?q?)$");
